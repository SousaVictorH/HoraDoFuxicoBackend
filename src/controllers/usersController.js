...
module.exports = {
  ...
  async signUp(req, res) {
    try {
      const { socialId, name, birthDate, phoneNumber, avatar } = req.body;

      return res
        .status(201)
        .json(await SignUp({ socialId, name, birthDate, phoneNumber, avatar }));
    } catch (error) {
      const { statusCode } = error.error;
  ...
  async update(req, res) {
    try {
      const { id } = req.params;
      const { name, birthDate, phoneNumber, avatar } = req.body;

      const user = await UpdateUser({
        id,
        userData: { name, birthDate, phoneNumber, avatar },
      });
